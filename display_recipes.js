document.addEventListener("DOMContentLoaded", function () {
  ...
  const searchResults = document.getElementById("search-recipes");
    ...
    fetch(`get_recipes.php?query=${encodeURIComponent(query)}`)
      ...
      .then((recipes) => {
        ...
        if (recipes.length > 0) {
          ...
          searchResults.innerHTML = "<p>No recipes found.</p>"; // Display this when no recipes are returned
        }
      })
      .catch((error) => {
      ...
      fetch(`get_recipe_details.php?id=${recipeId}`)
        ...
        .then((recipe) => {
          if (recipe) {
            ...
            document.getElementById("modify-description").value =
              recipe.description;
            document.getElementById("modify-ingredients").value =
  ...
  document.getElementById("modify-save").addEventListener("click", function () {
    ...
    if (imageInput) {
      ...
      const currentImageSrc = document.getElementById(
        "modify-current-image"
      ).src;
